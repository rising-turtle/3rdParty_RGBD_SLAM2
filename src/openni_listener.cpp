  ...
  image_encoding_("rgb8")
{
  ...
  // extractor_.reset(createDescriptorExtractor(ps->get<std::string>("feature_extractor_type")));
}
...
void OpenNIListener::loadBag(std::string filename)
{
  ...
  { //bag will be destructed after this block (hopefully frees memory for the optimizer)
    ...
    // ROS_WARN("openni_listener.cpp: before send image, after iamBusy");
    ...
    BOOST_FOREACH(rosbag::MessageInstance const m, view)
    {
      ...
      // ROS_WARN("openni_listener.cpp: try to send image, counter is %d", counter);
      do{ 
        usleep(150);
        if(!ros::ok()) return;
      } while(pause_);
      // ROS_WARN("openni_listener.cpp: after pause, then try to construct topic!");
                                           ...
                                           const sensor_msgs::CameraInfoConstPtr& cam_info)
{
  ...
  // compute the time elapse for the whole process 
  ...
  // compute cost time, if concurrent_node_construction is true, this whole time is not right
  double end_time_ms = ros::Time::now().toSec()*1000.; 
  static int number_successful_added = 0; 
  // ROS_ERROR("openni_listenner.cpp: whole %d frame cost time %.4f ms", ++number_successful_added, end_time_ms - begin_time_ms);
}
...
void OpenNIListener::processNode(Node* new_node)
{
  ...
  { 
    ...
    static std::ofstream time_vro_go("time_vro_go.log"); 
