...
/** This file contains the methods of G2O_GraphManager concerned with transfering 
...
void G2O_GraphManager::sendAllClouds(bool threaded){
    if (ParameterServer::instance()->get<bool>("concurrent_io") && threaded) {
        QFuture<void> f1 = QtConcurrent::run(this, &G2O_GraphManager::sendAllCloudsImpl);
        //f1.waitForFinished();
    }
...
tf::StampedTransform G2O_GraphManager::computeFixedToBaseTransform(Node* node, bool invert)
{
...
// If QT Concurrent is available, run the saving in a seperate thread
void G2O_GraphManager::saveBagfileAsync(QString filename){
    if (ParameterServer::instance()->get<bool>("concurrent_io")) {
        QFuture<void> f1 = QtConcurrent::run(this, &G2O_GraphManager::saveBagfile, filename);
        //f1.waitForFinished();
    }
    else {// Otherwise just call it without threading
...
void G2O_GraphManager::saveBagfile(QString filename)
{
...
void G2O_GraphManager::sendAllCloudsImpl()
{
  ScopedTimer s(__FUNCTION__);
...
// If QT Concurrent is available, run the saving in a seperate thread
void G2O_GraphManager::saveAllClouds(QString filename, bool threaded){
    if (ParameterServer::instance()->get<bool>("concurrent_io") && threaded) {
        QFuture<void> f1 = QtConcurrent::run(this, &G2O_GraphManager::saveAllCloudsToFile, filename);
        //f1.waitForFinished();
    }
    else {// Otherwise just call it without threading
...
void G2O_GraphManager::saveIndividualClouds(QString filename, bool threaded){
  if (ParameterServer::instance()->get<bool>("concurrent_io") && threaded) {
    QFuture<void> f1 = QtConcurrent::run(this, &G2O_GraphManager::saveIndividualCloudsToFile, filename);
    //f1.waitForFinished();
  }
  else {
...
bool G2O_GraphManager::updateCloudOrigin(Node* node)
{
    if(!node->valid_tf_estimate_) {
      ROS_INFO("Skipping node %i: No valid estimate", node->id_);
      return false;
...
void G2O_GraphManager::saveOctomap(QString filename, bool threaded){
  if (ParameterServer::instance()->get<bool>("octomap_online_creation")) {
    ...
    if (ParameterServer::instance()->get<bool>("concurrent_io") && threaded) {
      ...
      QFuture<void> f1 = QtConcurrent::run(this, &G2O_GraphManager::saveOctomapImpl, filename);
      //f1.waitForFinished();
    }
    else {
      saveOctomapImpl(filename);
...
void G2O_GraphManager::saveOctomapImpl(QString filename)
{
  ScopedTimer s(__FUNCTION__);
...
void G2O_GraphManager::writeOctomap(QString filename) const
{
    ScopedTimer s(__FUNCTION__);
    co_server_.save(qPrintable(filename));
}

void G2O_GraphManager::renderToOctomap(Node* node)
{
    ScopedTimer s(__FUNCTION__);
...
void G2O_GraphManager::saveIndividualCloudsToFile(QString file_basename)
{
  ScopedTimer s(__FUNCTION__);
...
void G2O_GraphManager::saveAllFeatures(QString filename, bool threaded)
{
    if (ParameterServer::instance()->get<bool>("concurrent_io") && threaded) {
        QFuture<void> f1 = QtConcurrent::run(this, &G2O_GraphManager::saveAllFeaturesToFile, filename);
        //f1.waitForFinished();
    }
    else {// Otherwise just call it without threading
...
void G2O_GraphManager::saveAllFeaturesToFile(QString filename)
{
    ScopedTimer s(__FUNCTION__);
...
void G2O_GraphManager::saveAllCloudsToFile(QString filename){
    ScopedTimer s(__FUNCTION__);
    if(graph_.size() < 1){
...
void G2O_GraphManager::pointCloud2MeshFile(QString filename, pointcloud_type full_cloud)
{
...
void G2O_GraphManager::saveTrajectory(QString filebasename, bool with_ground_truth)
{
    ScopedTimer s(__FUNCTION__);
...
void G2O_GraphManager::visualizeFeatureFlow3D(unsigned int marker_id, bool draw_outlier)
{
    ScopedTimer s(__FUNCTION__);
...
void G2O_GraphManager::visualizeGraphEdges() const {
    ScopedTimer s(__FUNCTION__);
...
void G2O_GraphManager::visualizeGraphNodes() const {
    ScopedTimer s(__FUNCTION__);
...
void G2O_GraphManager::saveG2OGraph(QString filename)
{
  ROS_INFO("Saving G2O graph to %s", qPrintable(filename));
  optimizer_->save(qPrintable(filename));
}

tf::StampedTransform G2O_GraphManager::stampedTransformInWorldFrame(const Node* node, const tf::Transform& computed_motion) const 
{
    std::string fixed_frame = ParameterServer::instance()->get<std::string>("fixed_frame_name");
    std::string base_frame  = ParameterServer::instance()->get<std::string>("base_frame_name");
    if(base_frame.empty()){ //if there is no base frame defined, use frame of sensor data
...
void G2O_GraphManager::broadcastLatestTransform(const ros::TimerEvent& event) const
{
...
void G2O_GraphManager::broadcastTransform(const tf::StampedTransform& stamped_transform) const 
{
    br_.sendTransform(stamped_transform);
}

/*
void G2O_GraphManager::broadcastTransform(Node* node, tf::Transform& computed_motion){
    std::string fixed_frame = ParameterServer::instance()->get<std::string>("fixed_frame_name");
    std::string base_frame  = ParameterServer::instance()->get<std::string>("base_frame_name");
    if(base_frame.empty()){ //if there is no base frame defined, use frame of sensor data
      base_frame = node->header_.frame_id;
...
void G2O_GraphManager::drawFeatureFlow(cv::Mat& canvas, cv::Scalar line_color,
