 ...
 */
#ifndef G2O_PARAMETER_SERVER_H_
#define G2O_PARAMETER_SERVER_H_
#include <string>
#include <ros/ros.h>
 ...
 */
class G2O_ParameterServer {
public:
    /*!
     * Returns the singleton instance
     */
    static G2O_ParameterServer* instance();
    ...
    void set(const std::string param, T value) {
        ...
        if(config.count(param)==0){
          ROS_ERROR("G2O_ParameterServer: Ignoring invalid parameter: \"%s\"", param.c_str());
          return;
        }
#endif
        config[param] = value;
        setOnG2O_ParameterServer(pre+param, value);
    }
...
#ifdef RGBDSLAM_DEBUG
        if(config.count(param)==0){
          ROS_FATAL("G2O_ParameterServer object queried for invalid parameter \"%s\"", param.c_str());
...
private:
    ...
    static G2O_ParameterServer* _instance;
    std::string pre;
     ...
     */
    template<typename T>
    T getFromG2O_ParameterServer(const std::string param, T def) {
        T result;
    ...
    template<typename T>
    void setOnG2O_ParameterServer(const std::string param, T new_val) {
