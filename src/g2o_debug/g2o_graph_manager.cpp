...
// G2O_GraphManager::G2O_GraphManager(){}
    ...
    current_backend_("none")
{
  ...
  std::string fixed_frame = G2O_ParameterServer::instance()->get<std::string>("fixed_frame_name");
  std::string base_frame  = G2O_ParameterServer::instance()->get<std::string>("base_frame_name");
    
  latest_transform_cache_ = tf::StampedTransform(tf::Transform::getIdentity(), ros::Time::now(), base_frame, fixed_frame);
//  timer_ = nh.createTimer(ros::Duration(0.1), &G2O_GraphManager::broadcastLatestTransform, this);
}


void G2O_GraphManager::setOptimizerVerbose(bool verbose){
    optimizer_->setVerbose(verbose);
}

void G2O_GraphManager::createOptimizer(std::string backend, g2o::SparseOptimizer* optimizer)
{
...
/*  SlamBlockSolver* solver = NULL;
  if(backend == "cholmod" || backend == "auto"){
      ROS_INFO("I am in choldmod and auto");
...
QList<int> G2O_GraphManager::getPotentialEdgeTargetsWithDijkstra(const Node* new_node, int sequential_targets, int geodesic_targets, int sampled_targets, int predecessor_id, bool include_predecessor)
{
...
/*
void G2O_GraphManager::resetGraph(){
    #ifdef DO_FEATURE_OPTIMIZATION
     next_landmark_id = 0;
     landmarks.clear();
    #endif
...
void G2O_GraphManager::firstNode(Node* new_node) 
{
...
bool G2O_GraphManager::nodeComparisons(Node* new_node, 
...
void G2O_GraphManager::localizationUpdate(Node* new_node, QMatrix4x4 motion_estimate)
{
...
bool G2O_GraphManager::addNode(Node* new_node) 
{
...
void G2O_GraphManager::addKeyframe(int id)
{
  ScopedTimer s(__FUNCTION__);
...
bool G2O_GraphManager::addEdgeToG2O(const LoadedEdge3D& edge,Node* n1, Node* n2,  bool largeEdge, bool set_estimate, QMatrix4x4& motion_estimate) {
    ScopedTimer s(__FUNCTION__);
    assert(n1);
    assert(n2);
...
double G2O_GraphManager::optimizeGraph(double break_criterion, bool nonthreaded, QString filebasename){
  if(ParameterServer::instance()->get<bool>("concurrent_optimization") && !nonthreaded) {
    ROS_DEBUG("Optimization done in Thread");
    QtConcurrent::run(this, &G2O_GraphManager::optimizeGraphImpl, break_criterion); 
    return -1.0;
  }
  else { //Non-concurrent
...
double G2O_GraphManager::optimizeGraphImpl(double break_criterion)
{
  ...
  if(!optimization_mutex_.tryLock(2)) //ms
  {
...
void G2O_GraphManager::deleteCameraFrame(int id)
{
    QMutexLocker locker(&optimizer_mutex_);
    QMutexLocker locker2(&optimization_mutex_);
...
unsigned int G2O_GraphManager::pruneEdgesWithErrorAbove(float thresh){
    QMutexLocker locker(&optimizer_mutex_);
    QMutexLocker locker2(&optimization_mutex_);
...
QList<QPair<int, int> >* G2O_GraphManager::getGraphEdges()
{
    ScopedTimer s(__FUNCTION__);
...
QList<QMatrix4x4>* G2O_GraphManager::getAllPosesAsMatrixList(){
    ScopedTimer s(__FUNCTION__);
...
void G2O_GraphManager::reducePointCloud(pointcloud_type const * pc) {
...
void G2O_GraphManager::printEdgeErrors(QString filename){
  QMutexLocker locker(&optimizer_mutex_);
  std::fstream filestr;
...
double G2O_GraphManager::geodesicDiscount(g2o::HyperDijkstra& hypdij, const MatchingResult& mr){
...
void G2O_GraphManager::sanityCheck(float thresh){ 
...
void G2O_GraphManager::filterNodesByPosition(float x, float y, float z)
{
...
void G2O_GraphManager::occupancyFilterClouds(){
