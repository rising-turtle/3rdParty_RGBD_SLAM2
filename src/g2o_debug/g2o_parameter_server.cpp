...
G2O_ParameterServer* G2O_ParameterServer::_instance = NULL;

void G2O_ParameterServer::defaultConfig() {
  ...
  addOption("bagfile_name",                  std::string("/home/davidz/work/data/TUM_kinect_benchmark/rgbd_dataset_freiburg1_desk.bag"),  "Read data from a bagfile, make sure to enter the right topics above");
  ...
  addOption("optimizer_skip_step",           static_cast<int> (2),                      "Optimize every n-th frame. Set negative for offline operation ");
...
G2O_ParameterServer::G2O_ParameterServer() {
...
G2O_ParameterServer* G2O_ParameterServer::instance() {
  if (_instance == NULL) {
    _instance = new G2O_ParameterServer();
  }
  return _instance;
}

void G2O_ParameterServer::addOption(std::string name, boost::any value, std::string description){
    config[name] = value;
    descriptions[name] = description;
}

/* Used by GUI */
std::string G2O_ParameterServer::getDescription(std::string param_name) {
  return descriptions[param_name];
}

void G2O_ParameterServer::getValues() {
  ...
  for (itr = config.begin(); itr != config.end(); ++itr) {
    ...
    if (itr->second.type() == typeid(std::string)) {
      config[name] = getFromG2O_ParameterServer<std::string> (pre + name,
      ...
      config[name] = getFromG2O_ParameterServer<int> (pre + name,
      ...
      config[name] = getFromG2O_ParameterServer<double> (pre + name,
      ...
      config[name] = getFromG2O_ParameterServer<bool> (pre + name,
...
void G2O_ParameterServer::checkValues() {
