...
void ParameterServer::defaultConfig() {
  ...
  addOption("feature_detector_type",         std::string("SIFT"),                       "SIFTGPU, SURF, SIFT or ORB, or variants like GridDynamicORB or DynamicSURF (however, not GridXXX only, and not in combination with SIFTGPU)");
  addOption("feature_extractor_type",        std::string("SIFT"),                       "SIFTGPU, SURF, SIFT or ORB");
  ...
  addOption("emm__skip_step",                static_cast<int> (0),                      "When evaluating the transformation, subsample rows and cols with this stepping");
  ...
  addOption("optimizer_skip_step",           static_cast<int> (20),                      "Optimize every n-th frame. Set negative for offline operation ");
  ...
  addOption("glwidget_without_clouds",       static_cast<bool> (true),                 "3D view should only display the graph");
