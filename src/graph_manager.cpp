...
double GraphManager::optimizeGraphImpl(double break_criterion)
{
  ...
  else //Got the lock
  {
   ...
   if (ps->get<std::string>("pose_relative_to") == "inaffected") {
     ...
     // ROS_ERROR("graph_manager.cpp: before initializeOptimization(cam_cam_edges_)");
     // optimizer_->initializeOptimization(cam_cam_edges_);
     optimizer_->initializeOptimization();
     // ROS_ERROR("graph_manager.cpp: after initializeOptimization(cam_cam_edges_)");
   }
    ...
    //Optimize to convergence
      ...
      do {
        ...
        currentIt += optimizer_->optimize(1);//optimize 5 10 iterations per step
        optimizer_->computeActiveErrors();
        chi2 = optimizer_->chi2();
